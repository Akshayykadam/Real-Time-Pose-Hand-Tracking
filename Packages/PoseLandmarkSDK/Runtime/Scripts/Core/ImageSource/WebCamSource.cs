...
#pragma warning restore IDE0025
    ...
    private IEnumerator Initialize()
    {
      ...
      if (availableSources != null && availableSources.Length > 0)
      {
        ...
        var frontFacing = availableSources.Where(d => d.isFrontFacing).ToArray();
        webCamDevice = frontFacing.Length > 0 ? frontFacing[0] : availableSources[0];
      }
    }
    ...
    private IEnumerator GetPermission()
    {
      lock (_PermissionLock)
      {
        ...
        if (!Permission.HasUserAuthorizedPermission(Permission.Camera))
        {
          ...
          // Wait until the user grants permission (or we timeout in a real scenario, but safely wait here)
